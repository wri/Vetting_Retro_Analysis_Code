...
for (theme in TFL_themes$thematic_area) {
  ...
  theme_cols <- strsplit(theme_cols, ",", simplify = TRUE)[[1]]
  
  valid_cols <- intersect(theme_cols, names(TF_RDD))
  if (length(valid_cols) > 0) {
    theme_data <- TF_RDD[, valid_cols]  # Define theme_data only once
    project_outcome <- "project_outcome"
    
  model <- rdrobust(theme_data, project_outcome ~ ., fuzzy = TRUE)
  if (!is.null(model)) {  # Check if model is not null
    print(model)
  } else {
